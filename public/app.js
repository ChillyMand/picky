...
import { clearProgress, copyText } from '/modules/browser-compat.js';
...
async function createSession() {
  const response = await fetch('/api/sessions', { method: 'POST', headers: { 'content-type': 'application/json' }, body: JSON.stringify({ pairCode: pendingPairCode || null }) });
...
function showResult() {
  ...
  const pairSection = match ? `<div class="result-card pair-report"><h2>你们的饭桌匹配度</h2><div class="pair-score">${match.score}%</div><p class="verdict">${escapeHtml(match.verdict)}</p><p>共同回答 ${match.overlapCount} 道题</p><div class="pair-lists"><div><h3>😋 都爱吃</h3><p>${match.sharedLikes.map((food) => escapeHtml(food.name)).join('、') || '暂时没有'}</p></div><div><h3>🤝 一起避开</h3><p>${match.sharedAvoids.map((food) => escapeHtml(food.name)).join('、') || '暂时没有'}</p></div><div><h3>⚡ 饭桌分歧</h3><p>${match.conflicts.map((food) => escapeHtml(food.name)).join('、') || '几乎没有'}</p></div></div></div>` : `<div class="pair-code-card"><small>你的 4 位好友配对码</small><strong>${escapeHtml(state.publicCode || '----')}</strong><p>发给好友一起测；双方完成后，就能查看你们的饭桌默契度。</p><input id="result-friend-code" maxlength="4" autocomplete="off" placeholder="输入对方配对码" aria-label="输入对方配对码"><div class="pair-actions"><button data-action="copy-pair">复制配对码</button><button data-action="lookup-result-match">查看匹配度</button></div></div>`;
