...
async function loadSessions() {
  ...
  $('#empty').hidden = sessions.length > 0; $('#session-rows').innerHTML = sessions.map((session) => `<tr data-id="${session.id}"><td>${date(session.startedAt)}</td><td><code>${session.id.slice(0, 8)}</code></td><td>${session.result ? `<span class="score">${session.result.pickyScore}</span> ${escapeHtml(session.result.personality.name)}` : '未生成结果'}</td><td>${escapeHtml(session.ip || '--')} · ${escapeHtml(session.device?.model || session.device?.type || '未知')} · ${escapeHtml(session.device?.browser || '未知')}</td><td>${session.answers.length}</td><td><span class="status ${session.status}">${session.status === 'completed' ? '已完成' : '进行中'}</span></td></tr>`).join('');
}

async function openDetail(id) {
  const session = await fetch(`/api/admin/sessions/${id}`).then((response) => response.json());
  const result = session.result;
  $('#drawer-content').innerHTML = `<p class="eyebrow">SESSION DETAIL</p><h2>${result ? escapeHtml(result.personality.name) : '未完成测试'}</h2><div class="detail-grid"><div class="detail-box"><small>记录 ID</small><b>${session.id}</b></div><div class="detail-box"><small>访客 ID</small><b>${escapeHtml(session.visitorId)}</b></div><div class="detail-box"><small>开始时间</small><b>${date(session.startedAt)}</b></div><div class="detail-box"><small>完成时间</small><b>${date(session.completedAt)}</b></div><div class="detail-box"><small>挑食指数</small><b>${result?.pickyScore ?? '--'}</b></div><div class="detail-box"><small>题数</small><b>${session.answers.length}</b></div></div><h3>完整选择</h3><div class="answers-list">${session.answers.sort((a, b) => a.order - b.order).map((answer) => { const food = getFoodById(answer.foodId); return `<div class="answer-row"><span class="emoji">${food?.emoji || '🍽️'}</span><b>${escapeHtml(food?.name || answer.foodId)}</b><span>${choiceNames[answer.choice]}</span><small>${Math.round((answer.durationMs || 0) / 1000)}s</small></div>`; }).join('') || '<p>暂无选择。</p>'}</div><h3>设备环境</h3><div class="detail-grid"><div class="detail-box"><small>IP</small><b>${escapeHtml(session.ip || '--')}</b></div><div class="detail-box"><small>设备</small><b>${escapeHtml(session.device?.model || '未知')} / ${escapeHtml(session.device?.type || '未知')}</b></div><div class="detail-box"><small>系统</small><b>${escapeHtml(session.device?.os || '未知')}</b></div><div class="detail-box"><small>浏览器</small><b>${escapeHtml(session.device?.browser || '未知')}</b></div><div class="detail-box"><small>屏幕</small><b>${session.viewport?.width || '--'} × ${session.viewport?.height || '--'}</b></div><div class="detail-box"><small>语言 / 时区</small><b>${escapeHtml(session.language || '--')} / ${escapeHtml(session.timezone || '--')}</b></div></div><h3>User-Agent</h3><p class="raw">${escapeHtml(session.userAgent || '--')}</p>`;
